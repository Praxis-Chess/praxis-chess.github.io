/* ==========================================================================
   ...
   WHY IT EXISTS. Every performance number produced on the machine that wrote
   the code is worthless as an absolute — it can rank two things against each
   other and nothing more. Before anything here is optimised, the before
   figure has to come off the machine that is actually slow. This is the
   instrument that produces it.

   IT REPORTS p95, NOT JUST THE MEAN. A page can average 60fps and still feel
   broken, because what a person notices is the worst frames, not the typical
   one, and the mean hides exactly the thing being hunted.

   IT ALSO REPORTS WHAT IS ASLEEP, which is the other half of a useful
   measurement: 120fps with the hero field dormant is not a result, and
...
window.PAGE.register('fps', function () {
  ...
  function tick(now) {
    ...
    if (dt <= 0 || dt > 1000) return;        // a tab waking up is not a frame
    ...
    sorted.set(times);
    var view = Array.prototype.slice.call(sorted, 0, filled)
    ...
    if (D.scrolled) st.push('scrolled past');
